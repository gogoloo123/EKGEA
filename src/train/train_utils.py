...
def cos_sim_mat_generate(emb1, emb2, device, bs=128):
    ...
    emb1.to(device)
    emb2.to(device)
    array_emb1 = F.normalize(torch.FloatTensor(emb1), p=2, dim=1) # 对张量进行归一化操作，p表示归一化的范数类型，dim=1表示对行维度进行归一化
    array_emb2 = F.normalize(torch.FloatTensor(emb2), p=2, dim=1)
    res_mat = batch_mat_mm(array_emb1, array_emb2.t(), device, bs=bs)
    return res_mat # 返回余弦相似矩阵
...
def batch_mat_mm(mat1, mat2, device, bs=128):
    ...
    axis_0 = mat1.shape[0]
    # mat2.to(device)
    for i in range(0, axis_0, bs): # 从0到2033，隔128取一个数字
        temp_div_mat_1 = mat1[i:min(i + bs, axis_0)]
        res = temp_div_mat_1.mm(mat2) # 进行矩阵乘法
