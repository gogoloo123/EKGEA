...
class PairwiseTrainer(MultiprocessingTool):
    def __init__(self):
        ...
        self.get_emb_batch = 512   # 获取嵌入向量的批次大小
        self.train_emb_batch = 8   # 训练嵌入向量的批次
        # self.emb_cos_batch = 512
        self.nearest_sample_num = 128  # 计算相似度的最近邻样本数
        self.neg_num = 2  # 训练嵌入向量的负样本数量
    ...
    def data_prepare(
        ...
        self.fs1 = fs1
        self.fs2 = fs2
        tokenizer = BertTokenizer.from_pretrained(args.pretrain_bert_path) # 加载预训练模型
        cls_token, sep_token = tokenizer.convert_tokens_to_ids(['[CLS]', '[SEP]']) # 接受一个词符列表作为输入，并返回对应的词索引列表，这里得到的是[CLS],[SEP]两个token的id
        freqs1 = BertDataLoader.load_freq(dataset1) # 加载每个token对应的id以及出现的次数，字典类型 {('##r', 1197): 2564, ('e', 174): 2521}
        freqs2 = BertDataLoader.load_freq(dataset2)
        freqs1 = {key[1]: value for key, value in freqs1.items()} # 字典类型，包含的数据是 -- token的id：当前token出现的次数
        ...
        eid2data1 = [self.tids_solver(item, cls_token=cls_token, sep_token=sep_token, freqs=None) for item in eid2tids1.items()] # 有多少实体，列表长度就为多少，列表中的元素含义是实体id，以及当前实体的token 和标记的tensor向量
        self.eid2data1 = {key: value for key, value in eid2data1} # 中文数据
        # self.eid2data1和self.eid2data2中存放的是中文和英文的所有实体的属性的token的tensor值
        ...
        eid2data2 = [self.tids_solver(item, cls_token=cls_token, sep_token=sep_token, freqs=None) for item in eid2tids2.items()] # 英文数据 而且在每个实体的token前都会加上[CLS]
        ...
        if not args.blocking:
            ...
            self.train_links = self.load_links(links.train, fs1.entity_ids, fs2.entity_ids) # 返回train_link种子实体对在entities1和entities2中的id对应关系
            ...
            self.train_links_p = [(e1, e2) for e1, e2 in self.train_links if e1 in self.eid2data1 and e2 in self.eid2data2] # 判断种子实体对是否在entities中
            ...
            '''
            self.block_loader1, self.block_loader2 = self.links_pair_loader(self.all_ent1s_p, self.all_ent2s_p) # 训练数据的加载器
            if VALID:
                self.valid_link_loader1, self.valid_link_loader2 = self.links_pair_loader(self.valid_ent1s_p, self.valid_ent2s_p) # 验证数据的加载器
            self.test_link_loader1, self.test_link_loader2 = self.links_pair_loader(self.test_ent1s_p, self.test_ent2s_p) # 测试数据的加载器
            '''
            获取ILL中的entity
            全部entity，作为初筛的候选实体
            顺序输入模型得到embedding
    ...
    def links_pair_loader(self, ent1s, ent2s):
        inputs1 = self.get_tensor_data(ent1s, self.eid2data1) # 返回值包含两个元素，第一个是输入序列，第二个是标记序列
        inputs2 = self.get_tensor_data(ent2s, self.eid2data2)
        ds1 = TensorDataset(*inputs1) # 将接收的张量组成一个数据集
        ds2 = TensorDataset(*inputs2)
        sampler1 = SequentialSampler(ds1) # 在实例化时创建一个按顺序生成样本索引的采样器，可以在训练过程中按顺序遍历数据集中的样本
        sampler2 = SequentialSampler(ds2)
        loader1 = DataLoader(ds1, sampler=sampler1, batch_size=self.get_emb_batch) # 加载数据
        loader2 = DataLoader(ds2, sampler=sampler2, batch_size=self.get_emb_batch)
        return loader1, loader2

    @staticmethod
    def get_tensor_data(ents: list, eid2data: dict):
        inputs = [eid2data.get(key) for key in ents] # 根据实体id获取当前实体的输入
        print(len(inputs))
        input_ids = [ids for ids, mask in inputs]
        input_ids = t.stack(input_ids, dim=0) # 对输入的token值的张量进行堆叠，（3146,129） -- 代表有3146个实体，每个实体嵌入的向量是129维的
        masks = [mask for ids, mask in inputs]
        masks = t.stack(masks, dim=0) # 对标记值进行堆叠，维度与上面相同
    ...
    def train(
        ...
        if PARALLEL: # 当模型较大或者数据较多时，使用多个gpu并行计算
            bert_model = t.nn.DataParallel(bert_model)
        bert_model.to(device)
        criterion = t.nn.MarginRankingLoss(MARGIN) # 基于边际排名的损失函数，MARGIN是边际大小，用于计算两个输入张量之间的差异，并将差异与边际进行比较，从而计算出相应的损失。
        optimizer = AdamW(bert_model.parameters(), lr=1e-5) # 优化器，用于在训练过程中更新模型参数。W表示使用了权重衰减来对模型参数进行正则化
        # import transformers
        ...
        self.get_hits(bert_model, self.test_link_loader1, self.test_link_loader2, device=device)# 在测试集上
        # max_hit1 = 0
        if VALID:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i, batch in tt.batches(lambda batch: len(batch[0][0])): # 进行梯度下降和反向传播
                optimizer.zero_grad()
    ...
    def get_hits(self, bert_model: t.nn.Module, link_loader1, link_loader2, device):
        valid_emb1s = self.get_emb_valid(link_loader1, bert_model, device=device) # 经过bert后的输出向量，维度为（2033,300），2033表示测试集中有这么多实体，300是自己设置的bert模型输出维度
    ...
    def generate_train_tups(self, bert_model, train_ent1s, train_ent2s, train_links, device):
        bert_model.eval()
        all_emb1s = self.get_emb_valid(self.block_loader1, bert_model, device=device) # self.block_loader1是训练数据的加载器，是所有实体及其属性 中文嵌入结果
        all_emb2s = self.get_emb_valid(self.block_loader2, bert_model, device=device) # 英文嵌入结果
        ...
        train_emb1s = all_emb1s[train_ent_idx1s] # 从所有的中文实体嵌入结果中取出训练集中的结果
        ...
        # train_emb2s = all_emb1s[train_ent_idx2s]
        train_emb2s = all_emb2s[train_ent_idx2s] # 英文实体中的训练集嵌入结果
    ...
    def get_candidate_dict(self, train_ents, train_embs, all_ents, all_embs, device):
        cos_sim_mat = cos_sim_mat_generate(train_embs, all_embs, device=device) # 余弦相似矩阵
    ...
    def get_emb_valid(loader: DataLoader, model: BertModel or t.nn.DataParallel, device='cpu') -> t.Tensor:
        ...
        results = []
        with t.no_grad(): # 不计算梯度，不反向传播
            model.eval() # 预测模式
            for i, batch in TrainingTools.batch_iter(loader, 'get embedding'): # 512行，129列，行表示batch_size取了512个实体
                # batch = tuple(tup.to(device) for tup in batch)
                emb = model(batch[0].to(device), batch[1].to(device)).cpu() # 进行前向传播
                # batch = tuple(tup.cpu() for tup in batch)
                results.append(emb)
                # t.cuda.empty_cache()
            embs = t.cat(results, dim=0) # 得到最后所有实体的嵌入值，（x，300）维度的张量
    ...
    def load_links(link_path, entity_ids1: dict, entity_ids2: dict):
        ...
        with open(link_path, 'r', encoding='utf-8') as rfile:
            links = MPTool.packed_solver(links_line).send_packs(rfile).receive_results() # 读取train_link文件中的所有种子对，并按照entities_id的值转换为对应的id形式，前面的id是中文数据里的，后面的id是英文数据里的
            links = list(filter(lambda x: x[0] is not None and x[1] is not None, links)) # 遍历列表，将满足lambda表达式的值保留
        return links
    ...
    def tids_solver(
        ...
        assert len(tids) > 0
        if freqs is None:
            tids = PairwiseTrainer.reduce_tokens(tids, max_len=seq_max_len) # 判断是否大于最大长度
        else:
            tids = PairwiseTrainer.reduce_tokens_with_freq(tids, freqs, max_len=seq_max_len)

        pad_length = seq_max_len - len(tids)
        input_ids = [cls_token] + tids + [pad_token] * pad_length # 输入的token的id
        masks = [1] * (len(tids) + 1) + [pad_token] * pad_length # 标记有值记为1
        ...
        input_ids = t.tensor(input_ids, dtype=t.long) # 将输入的token的id转换为tensor类型，每个向量都是129维的
        masks = t.tensor(masks, dtype=t.long) # 将输标记值转换为tensor类型，每个向量都是129维的
