...
from train.LSTMAttnNet import LSTMAttnNet
...
class RelationModel(t.nn.Module):
    def __init__(self, rel_count1, rel_count2, all_embed1, all_embed2, device):
        ...
        self.rnn = LSTMAttnNet(embedding_dim, rnn_hidden_dim, 1, dropout=0.2, device=device) # 一个LSTM层
        ...
        self.bn = t.nn.Sequential( # 按照顺序组织每个层，输入会按照定义的顺序通过每个层进行计算
            t.nn.BatchNorm1d(rnn_hidden_dim), # 对输入数据进行归一化处理
            # t.nn.BatchNorm1d(bert_output_dim + rnn_hidden_dim),
            # t.nn.BatchNorm1d(bert_output_dim),
            t.nn.Softsign(), # Softsign激活函数定义为：f(x) = x / (1 + |x|)
        ).to(device)
        # #########2021-11-17
        attr_out_dim = 300
    ...
    def forward(self, pe1s, pe2s, ne1s, ne2s, bpn1s, bpn2s, bnn1s, bnn2s, bpr1s, bpr2s, bnr1s, bnr2s):
        ...
        pr1s = self.get_rel_embeds(bpn1s, bpr1s, pe1s, self.rel_embedding1, self.ent_embedding1) # 经过了LSTM层
        pr2s = self.get_rel_embeds(bpn2s, bpr2s, pe2s, self.rel_embedding2, self.ent_embedding2)
        nr1s = self.get_rel_embeds(bnn1s, bnr1s, ne1s, self.rel_embedding1, self.ent_embedding1)
        nr2s = self.get_rel_embeds(bnn2s, bnr2s, ne2s, self.rel_embedding2, self.ent_embedding2)
        # #########2021-11-17
        ...
        pos_emb1 = self.combiner(pos_emb1) # MLP层
        ...
        # #########2021-11-18
        pos_score = t.nn.functional.pairwise_distance(pos_emb1, pos_emb2, p=self.score_distance_level, keepdim=True) # 用于计算两样本之间的欧式距离
        y_pred1 = t.cosine_similarity(pos_emb1, pos_emb2).reshape(pe1s.shape[0], 1) # 用于计算样本之间的余弦相似度
    ...
    def case_study(self, batch, rel_embedding: t.nn.Embedding, all_embed, mode) -> (t.Tensor, t.Tensor):
        with t.no_grad():
            ...
            weights = None
            final_embds = None
            if t.numel(h) != 0 and t.numel(neighbor_mask) != 0:
    ...
    def get_rel_embeds(self, batch_neighbors, batch_relations, batch_ent: t.Tensor, rel_embedding, all_embed: t.nn.Embedding):
        ...
        ones = t.ones(batch_neighbors.shape).cuda()
        zeros = t.ones(batch_neighbors.shape).cuda()
        neighbor_mask = t.where(batch_neighbors == pad_idx, ones, zeros) # 根据条件选择张量中的元素
