...
class RelationTrainer(MultiprocessingTool):
    ...
    def data_prepare(
        ...
        cls_token, sep_token = tokenizer.convert_tokens_to_ids(['[CLS]', '[SEP]']) # 将CLS和SEP两个符号转换成token_id,z在句子开始的时候加上CLS，在句子之间加上SEP，
    ...
    def links_pair_loader(self, ent1s, ent2s):
        ...
        ds1 = TensorDataset(*inputs1) # 通过组合多个张量作为输入来创建数据集
        ds2 = TensorDataset(*inputs2)
        sampler1 = SequentialSampler(ds1)
        sampler2 = SequentialSampler(ds2)# 创建一个按顺序采样的采样器（sampler），顺序采样器会按照数据集中样本的顺序依次生成索引，用于在训练过程中按顺序遍历数据集。
        loader1 = DataLoader(ds1, sampler=sampler1, batch_size=self.get_emb_batch) # 创建一个数据加载器（data loader），用于批量加载数据集，参数 batch_size 指定了每个批次中的样本数量
    ...
    def train(
        ...
        bert_model = ModelTools.load_model(basic_bert_path) # 从指定的文件路径 model_path 中加载已保存的 PyTorch 模型
        if PARALLEL:
            bert_model = t.nn.DataParallel(bert_model) # 将一个模型包装起来，使其能够在多个GPU上并行计算，从而加速模型训练过程。
        bert_model.to(device) # 将 bert_model 移动到指定的设备上，以便在该设备上进行计算
        bert_model.eval() # 将 bert_model 设置为评估模式。在评估模式下，模型会禁用一些具有随机性的操作
        criterion = t.nn.MarginRankingLoss(MARGIN) # 损失函数类，margin表示两个样本之间的最小间隔，如果相似度得分小于 MARGIN，则认为这两个样本之间存在差异，损失将增加。
        ...
        if os.path.exists(links.kb_prop_emb_1):
            ...
            t.save(all_embed1s_p, links.kb_prop_emb_1) # 将 PyTorch 的张量 all_embed1s_p 保存到文件 links.kb_prop_emb_1 中
        ...
        optimizer = Adam(rel_model.parameters(), lr=0.001, weight_decay=5e-4) #  Adam 优化器来更新 rel_model 中的参数，最小化损失函数
        ...
        if VALID:
            model_tool1 = ModelTools(3, 'max') # 监控模型性能并决定是否提前停止训练。
            model_tool2 = ModelTools(3, 'max')
        ...
        for epoch in range(1, epochs + 1):
            ...
            stop1 = stop2 = False
            for i, batch in tt.batches(lambda batch: len(batch[0])): # 获取批次数据中第一个元素的长度
                ...
                # if PARALLEL:
                #     loss = loss.mean()
                print(float(loss.cpu()), end='') # 将结果转移到cpu上计算
            ...
            if VALID:
                if epoch > 10 and stop1 and stop2:
    ...
    @staticmethod
    def get_emb_valid(loader: DataLoader, model: BertModel or t.nn.DataParallel, device='cpu') -> t.Tensor: # 获取模型的嵌入向量
        ...
        with t.no_grad():
            ...
            for i, batch in TrainingTools.batch_iter(loader, 'get embedding'):
                ...
                emb = model(batch[0].to(device), batch[1].to(device)).cpu() # 前向传播得到嵌入向量emb
                # batch = tuple(tup.cpu() for tup in batch)
                results.append(emb)
                # t.cuda.empty_cache()
            embs = t.cat(results, dim=0) # 将所有的嵌入向量连接起来
        embs.requires_grad = False # 不需要进行梯度计算
        return embs
    ...
    def case_study_(loader: DataLoader, model: RelationModel, rel_embedding, all_embed, all_ents, fs: KBStore, mode, device='cpu'):
        ...
        with t.no_grad():
            ...
            count = 0
            for batch in loader:
                _, weights = model.case_study(batch, rel_embedding, all_embed, mode) # _代表忽略第一个元素
                if weights is None:
                    continue
                old_count = count
                count += len(batch[0])
                ...
                ents, facts = batch
                # neighbor_weights = [(fs.entities[ent_id], [(fs.relations[fact[0]], fs.entities[fact[1]], float(t.nn.functional.softmax(weight_tensor[:, :len(fact_list)], dim=1)[0, i])) for i, fact in enumerate(fact_list)]) for ent_id, fact_list, weight_tensor in zip(ent_ids, facts, weights)]  # enumerate获得对应的索引和值,dim=1表示按行进行softmax运算
                neighbor_weights = list()
                for ent_id, fact_list, weight_tensor in zip(ent_ids, facts, weights):
                    if fact_list != None:
                        for i, fact in enumerate(fact_list):
                            neighbor_weights.append((fs.relations[fact[0]],fs.entities[fact[1]],float(t.nn.functional.softmax(weight_tensor[:, :len(fact_list)], dim=1)[0, i])))
                ent_neighbor_weights.extend(neighbor_weights) # 将neighbor_weights列表元素合并到ent_neighbor_weights中
    ...
    def get_emb_valid_r(loader: DataLoader, model: RelationModel, rel_embedding, all_embed, mode, device='cpu') -> t.Tensor:
        ...
        with t.no_grad():
            model.eval()
            for batch in loader:
                emb = model.get_valid_emb(batch, rel_embedding, all_embed, mode)
